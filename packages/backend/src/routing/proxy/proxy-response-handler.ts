...
export function recordSuccess(
  ...
  if (!meta.fallbackFromModel) {
    const origin = autofix?.outcome === 'healed' ? autofix.manifestOrigin : undefined;
    if (origin) {
      // The pre-heal failure was Manifest-blocked (no provider was ever
      // contacted), so the original row goes through the sole writer of Manifest
      // rows — provider/tier NULL, `error_code` stamped — rather than a
      // provider-attributed `auto_fixed` row under the healed model.
      recordSafely(
