  ...
  describe('recordManifestBlockedRequest', () => {
    ...
    it('writes the auto_fixed original of a healed Manifest-blocked request, backdated under the retry', async () => {
      ...
      expect(row).toMatchObject({
        // The rich `auto_fixed` collapses to canonical `failed`; `superseded`
        // marks it as the recovered original of the healed pair.
        status: 'failed',
        superseded: true,
        error_code: 'M302',
        error_origin: 'request',
      ...
      expect(row.autofix_decision).toMatchObject({ issueId: 'iss-1', healAttemptId: 'ha-1' });
      // Backdated ~1s so the successful retry row sorts directly above it.
    ...
    it('keeps the error row and stamps the audit when a heal did not clear the block', async () => {
      ...
      await recorder.recordManifestBlockedRequest(ctx, {
        ...
        autofix: {
          ...
          outcome: 'unfixable',
          original_http_status: 404,
          // Phoenix was consulted (issue id) but sent no patch, so no retry ran.
          chain: [
            {
              attempt: 0,
              origin: 'original',
      ...
      const row = insertMock.mock.calls[0][0];
      expect(row).toMatchObject({
        // No successful retry, so the row stays a plain failure (not superseded).
        status: 'failed',
        error_code: 'M302',
        superseded: false,
        // The Phoenix audit still rides the row via autofix_decision.
        autofix_decision: expect.objectContaining({ issueId: 'iss-miss' }),
      });
      // Without a retry there is no linked healed pair, so it is not "applied".
      expect(row.autofix_applied).toBeFalsy();
