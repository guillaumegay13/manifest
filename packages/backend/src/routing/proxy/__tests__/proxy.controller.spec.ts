  ...
  describe('Manifest-authored failures', () => {
    ...
    it('threads the Auto-fix audit of a failed M302 heal onto the stub row', async () => {
      const autofix = {
        ...
        // Phoenix was consulted (issue id) but produced no working patch.
        chain: [
          {
            attempt: 0,
            origin: 'original',
      ...
      expect(mockMessageRepo.insert).toHaveBeenCalledWith(
        expect.objectContaining({
          // Rich `error` collapses to canonical `failed`; the failed heal is not
          // superseded (no working retry) but still carries the Phoenix audit.
          status: 'failed',
          error_code: 'M302',
          superseded: false,
          autofix_decision: expect.objectContaining({ issueId: 'iss-302' }),
        }),
      );
    });
  });
