...
**Manifest-blocked M302s are healable too.** An explicit `model` that resolves to no connected model never reaches a provider — historically it returned the friendly M302 without Phoenix ever seeing it. Now `healOrRejectUnavailableModel` (`proxy.service.ts`) synthesizes the 404 a provider would return (`code: model_not_found`, `param: model`, fingerprint provider inferred from the model prefix, `manifest` for bare names) and runs it through the same `maybeHeal` gates; a patch that renames the model re-resolves routing from scratch and serves the repaired request. Recording keeps the Manifest-row invariant: the healed original is written by `recordManifestBlockedRequest` as an `auto_fixed` row with `error_code='M302'` and provider/tier NULL (`AutofixRecord.manifestOrigin` is the marker), the retry is a normal `ok` row under the healed route. Agents without Auto-fix (or a failed heal) get the friendly M302 exactly as before — a failed attempt just stamps its Phoenix audit on the error row.
