...
| GET/PATCH                 | `/api/v1/workspace/defaults`                    | Session/API Key                     | Workspace defaults for new agents (Auto-fix, recording)                                                     |
...
- `AUTOFIX_DEFAULT_ENABLED` — Pins the deployment default for agents that made no choice, overriding the cloud-ON / self-hosted-OFF rule. Only an exact `true`/`false` counts; anything else falls back to the mode default. The workspace **Agent defaults** toggle and any per-agent choice both outrank it. Config-as-code counterpart for installs whose operator never opens the dashboard.
...
**Defaults cascade, newest choice wins.** `agents.autofix_enabled` is **nullable**: `NULL` means "no explicit choice" and falls through to the next layer. `AutofixService.resolveEnabled()` resolves `agent flag ?? tenants.autofix_default_enabled ?? deployment default`, where the deployment default is `AUTOFIX_DEFAULT_ENABLED` when set to an exact `true`/`false`, else **ON in cloud, OFF in self-hosted** (via `isSelfHosted()`, computed once at boot). The middle layer is the **Agent defaults** card in Account Preferences (`GET/PATCH /api/v1/workspace/defaults`, `WorkspaceDefaultsService`). An explicit `true`/`false` on the agent always wins, so a workspace default never overrides a choice someone made on an agent.

**Recording resolves the same way**, one layer shorter: `agents.record_messages ?? tenants.recording_default_enabled ?? ON`. Migration `1801800000000` makes that column nullable but deliberately leaves every existing row's value alone — unlike the pre-feature Auto-fix `false`, a stored recording `false` may be a real user choice, so collapsing it would silently switch recording back on. Existing agents therefore all carry an explicit value and ignore the workspace default; only new agents inherit it. The `GET/PATCH …/autofix` endpoints return the _resolved_ effective value, so the UI shows the right default state without persisting one. Migration `1799000300000` drops the old blanket `false` default and resets pre-feature `false` rows to `NULL` so they inherit the mode default.
